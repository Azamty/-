...
本分支只改 V2 已有的“主旋律合并”候选选择。它读取已经冻结的 raw recognition 音符，保留原始音符、原 MIDI 和各乐器分谱，不覆盖源文件。算法阶段没有重跑 MuScriptor 或重新训练模型；root 验收时另外运行了正式 `HighAccuracyService` 主旋律 smoke，处理耗时为 63.64 秒，输入整曲约 266 秒，可生成 16 个 SVG 和 `ScoreMID`，这是链路验证，不是重新识别。另一个 49.63 秒的 vocal profile 参数试验没有改善，也没有接入生产。源样本基线为 `08bf188`，本说明对应的工作日期为 2026-09-12。
