...
本地 MIDI 渲染域的 production 输入使用独立的直接渲染器 `fluidsynth_direct_ms_basic_v1`：源 MIDI 直接送入项目固定的 FluidSynth 2.6.0 Windows x64 binary 和 `MS Basic.sf3`，不经过 MuseScore MIDI importer。10 个 generated MIDI 样本、10 个 MAESTRO 八小节片段和 5 个 special MIDI 样本统一使用 44.1 kHz、立体声、PCM16、gain `0.2`、reverb/chorus 关闭和固定 `0.25s` 尾部。每个输入都有 `render_manifest.json`，记录 FluidSynth/SoundFont 版本和 SHA-256、源 MIDI 的 pitch/onset/duration/velocity/tempo/meter 摘要、两次渲染哈希及 source event completeness；旧的 oscillator 只保留为历史诊断，不再作为这些 production input。MAESTRO 仍是本地 MIDI render domain，保留源 MIDI 的演奏时值和力度，但不代表原始钢琴录音的踏板、房间或音色。

MusicXML 标准化器保留同声部的合法 3:2 tuplets；若 MuseScore 在 48 TPQ 边界产生紧凑的 4/8 tick 细网格片段，会在每个成员的 nominal duration 为整数时写成显式 `3[` 或受审计的 `3:1[` fine-grid bracket。独立的 1、2、4、5 tick 音符或休止符使用一个显式 `3:1[` singleton bracket；它不消费相邻事件，保留原始起止 tick、音符/休止和 tie 字段，且谱面明确显示连音括号。该表示不移动事件边界，并在 `notation_grid_repairs` 记录 ratio、voice、原始/最终 tick、event id 和 movement=0；不能证明时仍失败。若 tie voice 重分配把一个原始、连续、ratio 一致的 bracket 拆到多个 ScoreVoice，只在每个 source event 一一保留、pitch multiset 完整、无重叠/空洞且 actual/nominal 总时值严格闭合时重组到唯一 tie-repair voice；重组不改起止 tick，并在 `alignment_report.json` 的 `tuplet_marker_repairs` 中记录原 voice、目标 voice、tick、source event ids、原始 marker 和闭合时值。缺少唯一目标、出现 overlap/gap 或 pitch/tie 不一致时写入 conflict 并保持失败。孤立 marker 只有在清除后普通 48 TPQ 时值可序列化时才清除；同声部 gap、嵌套或不支持的 ratio 仍会失败。

没有可证明 ratio 或完整时值的其它细网格事件，即使它与相邻事件恰好连续，也不会被推断成三连音或被静默舍入。jianpu-ly 无法表达这类普通 48 TPQ 原子时，渲染阶段会明确失败，并把 voice、MusicXML event id、起止 tick、音符/休止、前后片段、tie/tuplet 状态和小节剩余时值写入失败原因；这保留了输入边界，避免用无依据的记谱修复掩盖模型输出问题。
...
验收登记文件是 `fixtures/high_accuracy/benchmark_manifest.json`。当前清单登记 30 个选定 production case，另保留 5 个 PJS 孤立歌声诊断 case 和 `luv-letter` 本机完整性候选；脚本不会把音频或语料复制进 Git。30 个选定 case 包含 10 个固定 seed 合成样本、10 个 MAESTRO performance-MIDI 渲染片段、5 个 CCMusic 中文混合曲片段和 5 个专门 fixture。所有 case 都必须走真实 audio→MuScriptor/GAME→BeatNet raw→baseline/new 链路。MAESTRO MIDI 的 Score 指标资格在本次拍点审计中保持不变，但不含独立的音乐拍点/重拍标注；原先生成的固定 120 BPM tick grid 已标为 diagnostic-only，不再计入 beat/downbeat F1。因此 30-case beat gate 当前明确缺 10 条合格拍点标注，必须补充独立对齐标注或替换样本。
...
MAESTRO 10 条已有本地 archive/render cache；selection manifest 记录下载、许可、哈希、源 MIDI 区间和 clip/output 哈希。审计确认 clip annotation 从裁剪并移到零点的 MIDI 生成，并未误用完整 MIDI 的绝对时间；但所谓“八小节”边界实际只是固定 120 BPM performance transport tick 的 32-quarter 窗口。十条不同演奏因此得到完全相同的 0–16 秒拍点网格。FluidSynth 的固定 `0.25s` 只追加在尾部，首个可听样本仅比 MIDI 首音晚 `1.623–3.006 ms`，不会解释低 F1。详见 `docs/maestro-timeline-audit.md`。这些样本继续作为真实 MuScriptor/BeatNet production case；本次只修正 beat/downbeat 资格，在取得独立对齐拍点前不参加拍点 F1。
