...
MusicXML 标准化器保留同声部的合法 3:2 tuplets；若 MuseScore 在 48 TPQ 边界产生紧凑的 4/8 tick 细网格片段，会在每个成员的 nominal duration 为整数且无 tie/voice 歧义时写成显式 `3[` 或受审计的 `3:1[` fine-grid bracket。该表示不移动事件边界，并在 `notation_grid_repairs` 记录 ratio、voice、tick、event id 和 movement=0；不能证明时仍失败。若 tie voice 重分配把一个原始、连续、ratio 一致的 bracket 拆到多个 ScoreVoice，只在每个 source event 一一保留、pitch multiset 完整、无重叠/空洞且 actual/nominal 总时值严格闭合时重组到唯一 tie-repair voice；重组不改起止 tick，并在 `alignment_report.json` 的 `tuplet_marker_repairs` 中记录原 voice、目标 voice、tick、source event ids、原始 marker 和闭合时值。缺少唯一目标、出现 overlap/gap 或 pitch/tie 不一致时写入 conflict 并保持失败。孤立 marker 只有在清除后普通 48 TPQ 时值可序列化时才清除；同声部 gap、嵌套或不支持的 ratio 仍会失败。

没有 MusicXML tuplet marker 或 tie 依据的 1–2 tick 事件，即使它与相邻事件恰好连续，也不会被推断成三连音或被静默舍入。jianpu-ly 无法表达这类普通 48 TPQ 原子时，渲染阶段会明确失败，并把 voice、MusicXML event id、起止 tick、音符/休止、前后片段、tie/tuplet 状态和小节剩余时值写入失败原因；这保留了输入边界，避免用无依据的记谱修复掩盖模型输出问题。
