...
A direct offline synthesizer that consumes the original MIDI and this exact SoundFont could still be evaluated later, but no FluidSynth executable or Python binding is installed locally. Adding an unpinned dependency would not meet the reproducibility requirement. A future direct-synth pilot must record the executable/library version and hash, SoundFont hash, sample rate/format, and prove byte determinism plus exact MIDI event timing before any production raw is generated. Until then the current oscillator remains the recorded benchmark renderer; replacing it is a separate production change.
