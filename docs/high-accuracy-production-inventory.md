...
The registry keeps 40 cases with reliable reference MIDI, but marks exactly 30 for the current production gate: 10 deterministic known-MIDI renders, 10 ASAP score/performance-aligned piano clips, 5 CCMusic mixed-song segments, and 5 specialized deterministic segments. The 10 MAESTRO reference MIDIs remain reliable for pitch and event-time diagnostics; their fixed 120 BPM transport grids are not independent musical beat ground truth, so they are retained as `diagnostic_only` and excluded from the selected gate.
