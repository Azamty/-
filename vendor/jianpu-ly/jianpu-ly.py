...
# Local compatibility extension: jianpu-ly historically accepted only three
# octave marks in either direction.  The score pipeline keeps the original
# MIDI pitch, so a real low or high register can legitimately need more.  The
# Lilypond suffix is one octave closer to the centre than the jianpu dots
# because the generated MIDI staff is based on c' = MIDI 60.
def octaveLevel(octave):
    if not octave: return 0
    if any(mark not in "'," for mark in octave) or ("," in octave and "'" in octave):
        raise ValueError("octave marks must use one repeated comma or apostrophe")
    return -len(octave) if octave[0] == ',' else len(octave)

def lilyOctave(octave):
    level = octaveLevel(octave)
    if level >= 0: return "'" * (level + 1)
    return "," * max(0, -level - 1)

def octaveOffset(octave):
    count = abs(octaveLevel(octave))
    return 0 if not count else 1 + 0.6 * (count - 1)

def octaveDotsMarkup(octave, graceType=None):
    level = octaveLevel(octave)
    if not level: return ""
    count = abs(level)
    direction = "^" if level > 0 else "_"
    x_offset = 0.4 if graceType else 0.6
    if count == 1: dots = "."
    elif count == 2: dots = r"\two-dots"
    elif count == 3: dots = r"\three-dots"
    else:
        # Keep one visible dot per octave without extending the fixed
        # three-dot articulation table in the upstream script.
        dots = r'\markup { \bold "' + ("." * count) + r'" }'
    return r"-\tweak #'X-offset #%s %s%s " % (x_offset, direction, dots)
...
class NoteheadMarkup:
  ...
  def __call__(self,figures,nBeams,dots,octave,accidental,tremolo,word,line):
    ...
    # octave is any number of repeated apostrophes or commas
    # accidental is "", "#", "b"
    # tremolo is "" or ":32"
    ...
    if invisTieLast: # (so figures == "-")
        ...
        if any(mark not in "'," for mark in octave) or ("," in octave and "'" in octave): scoreError("Can't handle octave "+octave+" in",word,line)
        self.last_octave = octave
        self.octavesSeen.append(octaveLevel(octave)) # for figuring out the best clef with WithStaff
    ...
    if placeholder_chord.startswith("<"): ret += placeholder_chord  # chord in western or midi
        ...
        if not placeholder_chord=="r": ret += lilyOctave(octave) # so no-mark starts near middle C
    ...
    if not midi and not western and not '-' in figures:
      ...
      ret += oDict.get(octave, octaveDotsMarkup(octave, self.graceType))
    ...
    level = octaveLevel(octave)
    octave_marks = u"" if invisTieLast or not level else (u'\u0323' if level < 0 else u'\u0307') * abs(level)
    self.unicode_approx.append({'#':u"\u266f",'b':u"\u266d"}.get(0 if invisTieLast else accidental,u"")+(u"-" if invisTieLast else figures[-1:])+beamC+octave_marks+u''.join(c+beamC for c in dots)+(u"" if self.inBeamGroup else u" ")) # (NB inBeamGroup is correct only if not midi and not western)
...
def graceNotes_markup(notes,word,line,isAfter,harmonic=False):
    ...
    while i < len(notes):
        ...
        if n=='#':
            ...
            run = re.match(r"[,']+", notes[i:])
            octave = run.group(0) if run else n
            i += len(octave) - 1
        elif n in 'qsdh':
            beams = '*qsdh'.index(n)
            maxBeams=max(maxBeams,beams)
...
def chordNotes_markup(notes,word,line,graceType=None):
    ...
    for i in range(len(notes)):
        ...
        if n=='#':
            ...
            run = re.match(r"[,']+", notes[i:])
            octave = run.group(0) if run else n
            sortKey += (7 * len(octave) + 1) * (1 if n == "'" else -1)
        else:
            # number should be the last char of a note
    ...
    for f in dNotes:
        ...
        placeholder_chord += lilyOctave(f['octave']) + " "
    placeholder_chord += ">"
    ...
    baseline = 0
    for f in dNotes:
        offset = octaveOffset(f['octave'])
        if ',' in f['octave'] and (baseline or not graceType): baseline += offset/(2 if graceType else 1) # octaves below raises the baseline, except if the lowest one goes below the beam (non-gracenote chords can also have lowest octave below beam but height gets normalised)
        ret += ((r"\tweak #'Y-offset #%.1f " % baseline) if baseline else "")+(r'\note-mod-angka "' if not_angka else r'\note-mod "')+f['figure']+'" '+placeholders[f['figure']]+{"":"", "#":"is", "b":"es"}[f['accidental']]+lilyOctave(f['octave'])+" "
        if "," in f['octave']: ret += r"-\tweak #'Y-offset #%.1f " % (baseline -0.1 - (1.2 * offset)/(1.4 if graceType else 1))
        elif "'" in f['octave']: ret += r"-\tweak #'Y-offset #%.1f " % (baseline + (0.9+grace_height if graceType and baseline<1.5+grace_height else 1 if graceType else 1.6) + 0.02 * offset)
        ret += oDict.get(f['octave'], octaveDotsMarkup(f['octave'], graceType))+" "
        baseline += 1.5 if graceType and baseline>=1.5+grace_height else 1.5+grace_height if graceType else 2
        if "'" in f['octave']: baseline += offset/(2 if graceType else 1)
    ret += ">"
