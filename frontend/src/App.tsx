...
const highAccuracyArtifactLabel = (artifact: Artifact) => {
  const labels: Record<string, string> = {
    ...
    melody_harmony_score_json: "主旋律 + 伴奏和弦组合谱数据 JSON",
...
function App() {
  ...
  const renderNotationControls = (prefix: string) => <fieldset className="notation-options demucs-options" disabled={busy}>
    <legend>简谱整理</legend>
  ...
  const submitSelection = async (kind: "midi" | "score") => {
    ...
    try {
      const payload = { notation_engine: notationEngine, direct_options: directOptions(), selected_track_ids: selectedTrackIds, merge_main_melody: mergeMainMelody, bpm_override: Number(bpm) || 120, key_override: key, time_signature_override: meter };
