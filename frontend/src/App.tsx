...
function App() {
  ...
  const scoreArtifacts = artifacts
    .filter((item) => ["instrument_score_svg_long", "main_melody_svg_long", "instrument_score_svg", "main_melody_svg"].includes(item.kind))
    .sort((left, right) => {
      const longDelta = Number(right.kind.endsWith("_long")) - Number(left.kind.endsWith("_long"));
      return longDelta || (Number(left.page || 0) - Number(right.page || 0)) || left.artifact_id.localeCompare(right.artifact_id);
    });
  ...
  const vocalScoreArtifacts = artifacts
    .filter((item) => ["vocal_score_svg_long", "main_melody_svg_long", "score_svg_long", "vocal_score_svg", "main_melody_svg", "score_svg"].includes(item.kind))
    .sort((left, right) => {
      const longDelta = Number(right.kind.endsWith("_long")) - Number(left.kind.endsWith("_long"));
      return longDelta || (Number(left.page || 0) - Number(right.page || 0)) || left.artifact_id.localeCompare(right.artifact_id);
    });
  const vocalLongScoreArtifacts = vocalScoreArtifacts.filter((item) => item.kind.endsWith("_long"));
  const vocalPagedScoreArtifacts = vocalScoreArtifacts.filter((item) => !item.kind.endsWith("_long"));
