...
import { scoreToPlaybackData, type ScorePlaybackData, type ScorePlaybackNote } from "./scorePlayback";
...
type ScorePlaybackState = "idle" | "loading" | "playing" | "paused";
...
function App() {
  ...
  const [scorePlaybackState, setScorePlaybackState] = useState<ScorePlaybackState>("idle");
  const [scorePlaybackArtifactId, setScorePlaybackArtifactId] = useState<string | null>(null);
  const [scorePlaybackTime, setScorePlaybackTime] = useState(0);
  const [scorePlaybackError, setScorePlaybackError] = useState("");
  ...
  const scoreCacheRef = useRef<Map<string, ScorePlaybackData>>(new Map());
  const scorePlaybackRef = useRef<{ artifactId: string; notes: ScorePlaybackNote[]; duration: number; program: number } | null>(null);
  const scoreOriginRef = useRef<number | null>(null);
  ...
  const scoreStopTimerRef = useRef<number | null>(null);
  ...
  const scoreArtifacts = artifacts.filter((item) => ["instrument_score_svg_long", "main_melody_svg_long", "instrument_score_svg", "main_melody_svg"].includes(item.kind)).sort((left, right) => Number(right.kind.endsWith("_long")) - Number(left.kind.endsWith("_long")));
  const longScoreArtifacts = scoreArtifacts.filter((item) => item.kind.endsWith("_long"));
  const pagedScoreArtifacts = scoreArtifacts.filter((item) => !item.kind.endsWith("_long"));
  ...
  const clearScoreClock = useCallback(() => {
    if (scoreTimerRef.current !== null) window.cancelAnimationFrame(scoreTimerRef.current);
    if (scoreStopTimerRef.current !== null) window.clearTimeout(scoreStopTimerRef.current);
    scoreTimerRef.current = null;
    scoreStopTimerRef.current = null;
    scoreOriginRef.current = null;
  }, []);
  ...
  const stopScorePlayback = useCallback(() => {
    ...
    clearScoreClock();
    haltSynth("stop");
    scorePlaybackRef.current = null;
    scorePositionRef.current = 0;
    scoreDurationRef.current = 0;
    setScorePlaybackState("idle");
    setScorePlaybackArtifactId(null);
    setScorePlaybackTime(0);
  }, [clearScoreClock, haltSynth]);

  const pauseScorePlayback = useCallback(() => {
    if (scorePlaybackState !== "playing" || !scorePlaybackArtifactId) return;
    const audioNow = audioContextRef.current?.currentTime;
    const elapsed = scoreOriginRef.current === null || audioNow === undefined
      ? scorePositionRef.current
      : playbackPosition(audioNow, scoreOriginRef.current, scoreDurationRef.current);
    scoreSessionRef.current += 1;
    clearScoreClock();
    haltSynth("stop");
    const bounded = clampPlaybackOffset(elapsed, scoreDurationRef.current);
    scorePositionRef.current = bounded;
    setScorePlaybackTime(bounded);
    setScorePlaybackState("paused");
  }, [clearScoreClock, haltSynth, scorePlaybackArtifactId, scorePlaybackState]);

  const startScoreClock = useCallback((artifactId: string, duration: number, startOffset: number, audioStartAt: number) => {
    const session = scoreSessionRef.current + 1;
    scoreSessionRef.current = session;
    const boundedOffset = clampPlaybackOffset(startOffset, duration);
    scoreDurationRef.current = duration;
    scorePositionRef.current = boundedOffset;
    scoreOriginRef.current = audioStartAt - boundedOffset;
    setScorePlaybackArtifactId(artifactId);
    setScorePlaybackTime(boundedOffset);
    setScorePlaybackState("playing");
    const tick = () => {
      if (scoreSessionRef.current !== session || scoreOriginRef.current === null) return;
      const audioNow = audioContextRef.current?.currentTime;
      if (audioNow === undefined) return;
      const rawElapsed = audioNow - scoreOriginRef.current;
      if (rawElapsed >= duration + 0.15) {
        stopScorePlayback();
        return;
      }
      const position = playbackPosition(audioNow, scoreOriginRef.current, duration);
      scorePositionRef.current = position;
      setScorePlaybackTime(position);
      scoreTimerRef.current = window.requestAnimationFrame(tick);
    };
    scoreTimerRef.current = window.requestAnimationFrame(tick);
    scoreStopTimerRef.current = window.setTimeout(() => {
      if (scoreSessionRef.current === session) stopScorePlayback();
    }, Math.max(0, duration - boundedOffset + 0.35) * 1000);
  }, [stopScorePlayback]);
  ...
  const loadJob = useCallback(async (jobId: string) => {
    if (loadedJobRef.current && loadedJobRef.current !== jobId) {
      stopScorePlayback();
    ...
    if (next.status === "selection_ready" || next.status === "vocal_ready" || next.status === "completed") {
  ...
  const chooseSource = (nextSource: SourceKind) => {
    if (nextSource !== source) stopScorePlayback();
  ...
  const submit = async () => {
    ...
    setBusy(true); setError(""); setNotice(""); stopScorePlayback(); scoreCacheRef.current.clear();
  ...
  const retry = async () => {
    ...
    setBusy(true); setError(""); stopScorePlayback(); scoreCacheRef.current.clear();
    try {
  ...
  const generateVocal = async () => {
    ...
    setBusy(true); setError(""); setNotice("正在把已分离人声送入 GAME；不会重新运行 Demucs。"); stopScorePlayback(); scoreCacheRef.current.clear();
    try {
  ...
  const toggleTrack = (trackId: string) => {
    if (synthPlaying || synthPaused || scorePlaybackState !== "idle") stopScorePlayback();
  ...
  const playLightweightScore = async (
    artifactId: string,
    data: ScorePlaybackData,
    fallbackReason: string,
    startOffset: number,
  ) => {
    const AudioContextConstructor = getAudioContextConstructor();
    if (!AudioContextConstructor) throw new SynthPlaybackError("lightweight_unsupported", "当前浏览器没有 Web Audio，无法启动简谱试听。");
    let context = audioContextRef.current;
    if (!context) {
      try {
        context = new AudioContextConstructor();
        ...
        throw new SynthPlaybackError("lightweight_unsupported", "浏览器无法创建简谱试听的音频上下文，请检查设备声音权限。");
      }
    }
    try {
      await context.resume();
    } catch {
      throw new SynthPlaybackError("audio_context_blocked", "浏览器没有启用简谱试听，请再次点击播放并允许声音。");
    }
    if (context.state !== "running") throw new SynthPlaybackError("audio_context_blocked", "浏览器仍将音频上下文保持为暂停，请再次点击播放。");
    const resumeAt = clampPlaybackOffset(startOffset, data.duration_sec);
    const scheduledNotes = slicePlaybackNotes(data.notes, resumeAt);
    const start = context.currentTime + 0.08;
    const waveforms: OscillatorType[] = ["sine", "triangle", "square", "sawtooth"];
    scheduledNotes.forEach((note) => {
      const oscillator = context!.createOscillator();
      const gain = context!.createGain();
      const when = start + note.start_sec;
      const until = start + Math.max(note.start_sec + 0.06, note.end_sec);
      const frequency = Math.max(55, Math.min(1800, 440 * Math.pow(2, (note.midi - 69) / 12)));
      oscillator.type = waveforms[Math.abs(data.program) % waveforms.length];
      oscillator.frequency.setValueAtTime(frequency, when);
      gain.gain.setValueAtTime(0.0001, when);
      gain.gain.exponentialRampToValueAtTime(Math.max(0.04, Math.min(0.16, note.velocity / 900)), when + 0.012);
      gain.gain.exponentialRampToValueAtTime(0.0001, Math.max(when + 0.035, until));
      oscillator.connect(gain).connect(context!.destination);
      oscillator.start(when);
      oscillator.stop(Math.max(when + 0.05, until + 0.04));
    ...
    if (!lightweightNodesRef.current.length) throw new SynthPlaybackError("lightweight_empty", "当前简谱在暂停位置后没有可播放的音符。");
    setScorePlaybackError(`${fallbackReason} 已改用轻量试听；播放依据是最终量化简谱。`);
    setSynthResource("轻量音色 · Web Audio 波形 · 最终简谱");
    startScoreClock(artifactId, data.duration_sec, resumeAt, start);
  };

  const playScore = async (artifact: Artifact, forceRestart = false) => {
    const artifactId = artifact.artifact_id;
    const scoreArtifact = artifacts.find((item) => {
      if (artifact.stem_id && item.stem_id === artifact.stem_id) {
        return item.kind === "instrument_score_json" || item.kind === "main_melody_json";
      }
      return !artifact.stem_id && item.kind === "score_json";
    });
    if (!scoreArtifact?.url) {
      setScorePlaybackError("没有找到这张分谱对应的 score.json，暂时无法播放。 ");
      return;
    }
    if (scorePlaybackArtifactId === artifactId && scorePlaybackState === "playing" && !forceRestart) {
      pauseScorePlayback();
      return;
    }
    if (scorePlaybackState === "loading") return;
    const isResume = !forceRestart && scorePlaybackArtifactId === artifactId && scorePlaybackState === "paused";
    const startOffset = isResume
      ? clampPlaybackOffset(scorePositionRef.current, scoreDurationRef.current)
      : 0;
    if (!isResume) stopScorePlayback();
    if (synthPlaying || synthPaused) stopScorePlayback();
    setScorePlaybackError("");
    setError("");
    setScorePlaybackArtifactId(artifactId);
    setScorePlaybackState("loading");
    const request = scoreSessionRef.current + 1;
    scoreSessionRef.current = request;
    let started = false;
    try {
      let data = scoreCacheRef.current.get(artifactId);
      if (!data) {
        const payload = await readResponse(await fetch(scoreArtifact.url));
        data = scoreToPlaybackData(payload);
        scoreCacheRef.current.set(artifactId, data);
      }
      if (scoreSessionRef.current !== request) return;
      scorePlaybackRef.current = { artifactId, notes: data.notes, duration: data.duration_sec, program: data.program };
      ...
      synth.programChange(channel, data.program);
      const start = context.currentTime + 0.08;
      slicePlaybackNotes(data.notes, startOffset).forEach((note) => {
        const when = start + note.start_sec;
        const until = start + Math.max(note.start_sec + 0.04, note.end_sec);
        synth.noteOn(channel, note.midi, note.velocity, { time: when });
        synth.noteOff(channel, note.midi, { time: until });
      });
      setSynthResource("MuseScore General · SpessaSynth / SF3 · 最终简谱");
      startScoreClock(artifactId, data.duration_sec, startOffset, start);
      started = true;
    } catch (caught) {
      if (scoreSessionRef.current !== request) return;
      const fallbackCodes = new Set([
        "secure_context_required", "audio_worklet_unsupported", "processor_load_failed",
        "soundfont_range_unsupported", "soundfont_range_invalid", "soundfont_chunk_failed",
        "soundfont_user_fallback", "soundfont_timeout", "soundfont_network", "soundfont_http",
        "soundfont_content_type", "soundfont_incomplete", "soundfont_invalid_body",
        "soundfont_size_mismatch", "synth_init_failed",
      ]);
      if (caught instanceof SynthPlaybackError && fallbackCodes.has(caught.code)) {
        try {
          const data = scorePlaybackRef.current?.artifactId === artifactId
            ? { notes: scorePlaybackRef.current.notes, duration_sec: scorePlaybackRef.current.duration, program: scorePlaybackRef.current.program }
            : scoreCacheRef.current.get(artifactId);
          if (!data) throw new Error("简谱数据不可用。");
          await playLightweightScore(artifactId, data, caught.message, startOffset);
          setError("");
          started = true;
          return;
        } catch (lightweightCaught) {
          const reason = lightweightCaught instanceof Error ? lightweightCaught.message : "未知原因";
          setScorePlaybackError(`${caught.message} 轻量试听也未能启动：${reason}`);
        }
      } else {
        setScorePlaybackError(caught instanceof Error ? caught.message : "简谱试听启动失败。 ");
      }
      setScorePlaybackState("idle");
      setScorePlaybackArtifactId(null);
      setScorePlaybackTime(0);
      setError(caught instanceof Error ? caught.message : "简谱试听启动失败。 ");
    } finally {
      if (scoreSessionRef.current === request && !started) setScorePlaybackState("idle");
    }
  };
  ...
  const submitSelection = async (kind: "midi" | "score") => {
    ...
    setBusy(true); setError(""); setNotice(kind === "midi" ? "正在登记选择版本并生成 MIDI…" : "正在登记选择版本并生成分谱…"); stopScorePlayback();
    try {
  ...
  const playSynth = async () => {
    ...
    if (scorePlaybackState !== "idle") stopScorePlayback();
  ...
  const scoreJsonFor = (artifact: Artifact) => artifacts.find((item) => {
    if (artifact.stem_id && item.stem_id === artifact.stem_id) {
      return item.kind === "instrument_score_json" || item.kind === "main_melody_json";
    }
    return !artifact.stem_id && item.kind === "score_json";
  ...
  const renderScoreCard = (artifact: Artifact, long = false) => {
    const active = scorePlaybackArtifactId === artifact.artifact_id;
    const jsonArtifact = scoreJsonFor(artifact);
    const actionLabel = scorePlaybackState === "loading" && active
      ? "准备中…"
      : scorePlaybackState === "playing" && active
        ? "暂停"
        : scorePlaybackState === "paused" && active
          ? "继续"
          : "播放简谱";
    return <div className={`score-card-wrap ${long ? "score-card-wrap-long" : ""}`} key={artifact.artifact_id}>
      <a className={`score-card ${long ? "score-long-card" : ""}`} href={artifact.url || "#"} target="_blank" rel="noreferrer">
        <span className="score-thumb"><img src={artifact.url || ""} alt={artifact.label} /></span>
        <strong>{artifact.label}</strong>
        <small>{long ? "纵向长图 SVG · 默认预览" : "分页 SVG · 下载或新窗口查看"}</small>
      </a>
      {jsonArtifact?.url && <div className="score-playback-actions">
        <button type="button" className={`score-play-button ${active && scorePlaybackState === "playing" ? "playing" : ""}`} onClick={() => void playScore(artifact)} disabled={scorePlaybackState === "loading"} aria-label={`${actionLabel}：${artifact.label}`}>{actionLabel}</button>
        {active && scorePlaybackState !== "loading" && <button type="button" className="score-stop-button" onClick={stopScorePlayback}>停止</button>}
        {active && <span className="score-playback-time" aria-live="polite">{formatDuration(scorePlaybackTime)} / {formatDuration(scoreDurationRef.current)}</span>}
        {active && scorePlaybackState === "paused" && <button type="button" className="score-replay-button" onClick={() => void playScore(artifact, true)}>从头播放</button>}
        {active && scorePlaybackError && <span className="score-playback-error" role="status">{scorePlaybackError}</span>}
      </div>}
    </div>;
  };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {isInstrumental && isReady && (
        <section className="instrument-workspace panel" aria-labelledby="workspace-heading" data-selection-policy="single-checkbox-controls-roll-synth-midi-score">
          <div className="panel-heading workspace-title">
            <div>
              <p className="section-kicker">04 · SELECTION READY</p>
              <h2 id="workspace-heading">全量识别完成，选出要留下的轨道</h2>
              <p className="heading-note">{trackCountLabel} · 勾选变化不会重新运行 MuScriptor。</p>
            </div>
            <span className="selection-revision">R{job?.v2?.selection_revision || 0}</span>
          </div>
          <div className="listen-strip">
            <div className="listen-card original-listen">
              <span className="listen-icon">◉</span>
              <div><strong>原曲试听</strong><small>音频文件的真实播放</small></div>
              {originalArtifact?.url ? <audio ref={audioRef} controls src={originalArtifact.url} onTimeUpdate={(event) => setOriginalTime(event.currentTarget.currentTime)} onEnded={() => setOriginalTime(0)} /> : <span className="listen-pending">识别后可用</span>}
            </div>
            <div className="listen-card synth-listen">
              <span className="listen-icon">⌁</span>
              <div><strong>本机合成试听</strong><small title={soundfontStatus?.sha256 || "官方 SF3 音色库"} aria-live="polite">{synthResource}</small></div>
              {soundfontCacheNotice && <small className="synth-cache-note" aria-live="polite">{soundfontCacheNotice}</small>}
              <button type="button" className={`listen-button ${synthPlaying ? "playing" : ""}`} onClick={() => void playSynth()} disabled={synthLoading}>{synthLoading ? "准备试听…" : synthPlaying ? "暂停" : synthPaused ? "播放" : "播放选中轨"}</button>
              {synthLoading && soundfontDownloading && <button type="button" className="lightweight-action" onClick={useLightweightNow}>立即使用轻量试听</button>}
              <span className="synth-time">{formatDuration(synthTime)} / {formatDuration(duration)}</span>
              {synthError && <p className="synth-diagnostic" role="alert">{synthError}</p>}
              {lightweightActive && !synthLoading && <button type="button" className="lightweight-action" onClick={reloadHighQuality}>重新加载高质量音色</button>}
            </div>
          </div>
          <div className="workspace-grid">
            <div className="roll-column">
              <div className="subhead"><div><span className="section-kicker">PIANO ROLL</span><strong>时间同步试听轨</strong></div><div className="roll-tools"><label><input type="checkbox" checked={rollVisible} onChange={(event) => setRollVisible(event.target.checked)} />显示</label><label>缩放 <input aria-label="钢琴卷帘缩放" type="range" min="1" max="4" step="0.5" value={rollZoom} onChange={(event) => setRollZoom(Number(event.target.value))} /></label></div></div>
              {renderPianoRoll()}
              <p className="roll-caption">播放原曲或本机合成时，米白播放头沿原始秒数移动。颜色只表示轨道，音符时间不经过简谱量化。简谱播放按钮会读取最终 score.json。</p>
            </div>
            <aside className="track-roster">
              <div className="subhead"><div><span className="section-kicker">TRACK ROSTER</span><strong>乐器清单</strong></div><span className="roster-count">{selectedTrackIds.length}/{tracks.length}</span></div>
              <div className="track-list">{tracks.map((track, index) => { const color = LANE_COLORS[index % LANE_COLORS.length]; const selected = selectedTrackIds.includes(track.track_id); return <div className={`track-row ${selected ? "selected" : ""}`} key={track.track_id}><span className="track-color" style={{ background: color }} /><label className="track-check"><input type="checkbox" checked={selected} onChange={() => toggleTrack(track.track_id)} /><span><strong>{track.label_zh}</strong><small>{track.instrument_group} · {track.note_count} notes · {track.is_drum ? "不生成简谱" : `program ${track.program}`}</small></span></label></div>; })}</div>
              {selectedTracks.some((track) => track.is_drum) && <p className="drum-note">鼓组保留在试听与选择 MIDI 中，但不会生成简谱。</p>}
              {!pitchedSelected && selectedTrackIds.length > 0 && <p className="drum-note">当前只选中鼓组：可以下载 MIDI，简谱按钮会保持禁用。</p>}
            </aside>
          </div>
          <div className="export-rail">
            <div className="analysis-advice" role="status"><div><strong>识别建议，可修改</strong><span>{analysisSuggestion ? `${analysisSuggestion.bpm} BPM · ${analysisSuggestion.key} · ${analysisSuggestion.time_signature}` : "等待原音分析"}</span></div>{analysisSuggestion?.candidates && <small>候选：BPM {analysisSuggestion.candidates.bpm?.join(" / ") || "—"} · 调性 {analysisSuggestion.candidates.key?.join(" / ") || "—"} · 拍号 {analysisSuggestion.candidates.time_signature?.join(" / ") || "—"}</small>}{analysisSuggestion?.warnings?.map((warning, index) => <small key={`analysis-warning-${index}`}>{warning}</small>)}</div>
            <div className="override-fields"><div><label htmlFor="export-bpm">BPM</label><input id="export-bpm" inputMode="decimal" value={bpm} onChange={(event) => updateSelectionField("bpm", event.target.value)} /></div><div><label htmlFor="export-key">调性</label><select id="export-key" value={key} onChange={(event) => updateSelectionField("key", event.target.value)}>{KEYS.map((item) => <option key={item}>{item}</option>)}</select></div><div><label htmlFor="export-meter">拍号</label><select id="export-meter" value={meter} onChange={(event) => updateSelectionField("meter", event.target.value)}>{METERS.map((item) => <option key={item}>{item}</option>)}</select></div></div>
            <label className="merge-option"><input type="checkbox" checked={mergeMainMelody} onChange={(event) => setMergeMainMelody(event.target.checked)} />另外生成单声部主旋律 <small>会丢失和声</small></label>
            <div className="export-actions"><button type="button" className="outline-action" onClick={() => void submitSelection("midi")} disabled={busy || selectedTrackIds.length === 0}>下载所选 MIDI</button><button type="button" className="primary-action compact" onClick={() => void submitSelection("score")} disabled={busy || !pitchedSelected}><span>生成分谱</span><strong>↗</strong></button></div>
          </div>
          {scoreArtifacts.length > 0 && <div className="score-deck"><div className="subhead"><div><span className="section-kicker">SCORE PAGES</span><strong>已选有音高乐器分谱</strong></div><span className="score-warning">鼓组不在分谱中</span></div><div className="score-grid">{longScoreArtifacts.map((artifact) => renderScoreCard(artifact, true))}</div>{pagedScoreArtifacts.length > 0 && <details className="score-pages"><summary>分页 SVG（{pagedScoreArtifacts.length} 页）</summary><div className="score-grid">{pagedScoreArtifacts.map((artifact) => renderScoreCard(artifact))}</div></details>}<div className="download-stack">{longScoreArtifacts.map((artifact) => <a className="download-line" key={`${artifact.artifact_id}-download`} href={artifact.url || "#"}>↓ 下载长图 SVG · {artifact.label}</a>)}{selectedZip?.url && <a className="download-line" href={selectedZip.url}>↓ 下载全部分页 SVG ZIP</a>}{selectedMidi && <a className="download-line" href={selectedMidi.url || "#"}>↓ 下载选择版本 MIDI · R{job?.v2?.selection_revision || 0}</a>}</div></div>}
          {originalMidi && <a className="download-line muted-line" href={originalMidi.url || "#"}>↓ 完整识别 MIDI（原始时间） · 含全部轨道</a>}
        </section>
      )}
      ...
      {source === "vocal" && job?.status === "completed" && <section className="vocal-result panel" aria-labelledby="vocal-heading"><div className="panel-heading"><div><p className="section-kicker">04 · GAME RESULT</p><h2 id="vocal-heading">人声主旋律已经展开</h2><p className="heading-note">GAME 只处理已分离的人声，结果保留为单一主旋律谱面。分离模型：{actualDemucsModel.label}（{actualDemucsModel.id}）。</p></div><span className="status-chip completed"><i />完成</span></div><div className="vocal-listen"><div><strong>原曲试听</strong>{originalArtifact?.url && <audio controls src={originalArtifact.url} />}</div><div><strong>已分离人声</strong>{vocalArtifact?.url && <audio controls src={vocalArtifact.url} />}</div></div><div className="score-grid">{artifacts.filter((item) => item.kind === "score_svg_long").map((artifact) => renderScoreCard(artifact, true))}</div><details className="score-pages"><summary>分页 SVG（{artifacts.filter((item) => item.kind === "score_svg").length} 页）</summary><div className="score-grid">{artifacts.filter((item) => item.kind === "score_svg").map((artifact) => renderScoreCard(artifact))}</div></details><div className="download-stack">{artifacts.filter((item) => item.kind === "score_svg_long").map((artifact) => <a className="download-line" key={`${artifact.artifact_id}-download`} href={artifact.url || "#"}>↓ 下载长图 SVG · {artifact.label}</a>)}{vocalMidi?.url && <a className="download-line" href={vocalMidi.url}>↓ 下载人声主旋律 MIDI</a>}{artifacts.filter((item) => ["jianpu_source", "lilypond_source"].includes(item.kind)).map((artifact) => <a className="download-line" key={artifact.artifact_id} href={artifact.url || "#"}>↓ {artifact.label}</a>)}</div></section>}
      {!fixtureMode && job && (job.status === "selection_ready" || job.status === "vocal_ready" || job.status === "completed") && <p className="retention-note">任务 {job.id} · 人声分离结果与生成阶段可在刷新后从本机任务目录恢复。</p>}
