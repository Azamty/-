import { NOTATION_KEYS, keyLabel, isScoreDownload } from "./notationPreferences";
...
import { canPersistSelection, jobUrlForHistory, parseSelectionDraft, resolveInitialJobId, resolveSelectionValues, SELECTION_DRAFT_SCHEMA, SELECTION_DRAFT_VERSION, selectionStorageKey, type ManualSelectionFields } from "./selectionState";
import { classifyMelodyHarmonyScoreArtifacts, classifyScoreArtifacts, filterArtifactsForSelectionRevision, findMelodyHarmonyMidi } from "./scoreArtifacts";
import { clampPlaybackOffset, cloneSoundfontBuffer, playbackPosition, slicePlaybackNotes } from "./synthPlayback";

type NotationEngine = "direct-jianpu" | "musescore-midi-import";
type DirectOptions = { beat_divisor?: 1 | 2; subdivisions?: 2 | 4 | 8; key_changes?: { bar: number; key: string }[] };
...
type SelectionSnapshot = {
  notation_advice?: { version?: number };
  bpm_override_explicit?: boolean;
  key_override_explicit?: boolean;
  time_signature_override_explicit?: boolean;
  notation_engine?: NotationEngine;
  direct_options?: DirectOptions;
  revision?: number;
...
type AnalysisSuggestion = {
  ...
  notation_advice?: { key_ambiguous?: boolean; reason?: string };
...
function App() {
  ...
  const [manualNotation, setManualNotation] = useState(false);
  const [notationEngine, setNotationEngine] = useState<NotationEngine>("direct-jianpu");
  const [beatDivisor, setBeatDivisor] = useState<1 | 2>(1);
  const [subdivisions, setSubdivisions] = useState<2 | 4 | 8>(4);
  const [keyChangesText, setKeyChangesText] = useState("");
  const notationHydratedRef = useRef<string | null>(null);
  ...
  const isSingleMelodyScoreArtifact = (artifact: Artifact) => artifact.kind.startsWith("main_melody");
  const longScoreArtifacts = instrumentScoreArtifacts.long.filter((artifact) => !isSingleMelodyScoreArtifact(artifact));
  const pagedScoreArtifacts = instrumentScoreArtifacts.paged.filter((artifact) => !isSingleMelodyScoreArtifact(artifact));
  const mainMelodyLongScoreArtifacts = instrumentScoreArtifacts.long.filter(isSingleMelodyScoreArtifact);
  const mainMelodyPagedScoreArtifacts = instrumentScoreArtifacts.paged.filter(isSingleMelodyScoreArtifact);
  const melodyHarmonyScoreArtifacts = classifyMelodyHarmonyScoreArtifacts(currentArtifacts, activeSelectionRevision);
  const melodyHarmonyLongScoreArtifacts = melodyHarmonyScoreArtifacts.long;
  const melodyHarmonyPagedScoreArtifacts = melodyHarmonyScoreArtifacts.paged;
  const melodyHarmonyMidi = findMelodyHarmonyMidi(currentArtifacts, activeSelectionRevision);
  ...
  const mainMelodyMidi = [...currentArtifacts].reverse().find((item) => item.kind === "main_melody_score_midi");
  ...
  const primaryArtifactDownloads = currentArtifacts
    .filter((item) => isScoreDownload(item.kind) && !item.kind.endsWith("_score_svg") && item.url)
  ...
  const renderHighAccuracyMeta = () => {
    ...
    return <div className="high-accuracy-meta" aria-label="高精度处理信息">
      {hasMetadata && <div className="engine-readout"><span>高精度链路</span><strong>{highAccuracyMetadata.notation_engine === "direct-jianpu" ? "直接简谱" : highAccuracyMetadata.notation_engine || "—"}</strong><small>{highAccuracyMetadata.beat_engine || "—"} · BeatNet {highAccuracyMetadata.beatnet_version || "—"}{highAccuracyMetadata.notation_engine !== "direct-jianpu" && <> · MuseScore {highAccuracyMetadata.musescore_version || "—"}</>} · {highAccuracyMetadata.score_ticks_per_quarter || "—"} TPQ</small></div>}
  ...
  const renderScoreCards = (items: Artifact[], long: boolean, description: string) => (
    <div className="score-grid" data-score-group={long ? "long" : "paged"}>
      {items.map((artifact) => <a className={`score-card ${long ? "score-long-card" : ""}`} key={artifact.artifact_id} href={artifact.url || "#"} target="_blank" rel="noreferrer">
  ...
  const renderPagedScoreDetails = (items: Artifact[], description: string) => items.length > 0 && (
    <details className="score-pages" data-score-group="paged">
      <summary>分页 SVG（{items.length} 页）</summary>
      {renderScoreCards(items, false, description)}
    </details>
  );

  const renderArtifactDownloads = () => {
    if (!primaryArtifactDownloads.length) return null;
    return <div className="artifact-download-panel">
      <div className="subhead"><div><span className="section-kicker">FILES</span><strong>下载简谱</strong></div><span className="score-warning">PDF · SVG · MIDI</span></div>
      <div className="artifact-download-grid">{primaryArtifactDownloads.map((artifact) => <a className="artifact-download-card" key={artifact.artifact_id} href={artifact.url || "#"} download={artifact.filename}><strong>{artifact.label}</strong><small>{artifact.filename} · {formatBytes(artifact.size_bytes)}</small></a>)}</div>
  ...
  const directOptions = (): DirectOptions => {
    if (notationEngine !== "direct-jianpu") return job?.v2?.direct_options || {};
    const key_changes = keyChangesText.trim() ? keyChangesText.split(/[,，;；]/).map((part) => {
      const match = part.trim().match(/^(\d+)\s*[:：]\s*([A-G][#b]?m?)$/);
      if (!match || Number(match[1]) < 1 || !KEYS.includes(match[2])) throw new Error("转调请填写“小节:调号”，例如 1:Eb, 56:E。");
      return { bar: Number(match[1]), key: match[2] };
    }) : [];
    return { beat_divisor: beatDivisor, subdivisions, key_changes };
  };

  const renderNotationControls = (prefix: string) => <details><summary>高级简谱设置（可选）</summary><fieldset className="notation-options demucs-options" disabled={busy}>
    <legend>简谱整理</legend>
    <div className="override-fields">
      <div><label htmlFor={`${prefix}-notation`}>生成方式</label><select id={`${prefix}-notation`} value={notationEngine} onChange={(e) => setNotationEngine(e.target.value as NotationEngine)}><option value="direct-jianpu">直接简谱（默认）</option><option value="musescore-midi-import">MuseScore 记谱</option></select></div>
      {notationEngine === "direct-jianpu" && <><div><label htmlFor={`${prefix}-speed`}>记谱速度</label><select id={`${prefix}-speed`} value={beatDivisor} onChange={(e) => setBeatDivisor(Number(e.target.value) as 1 | 2)}><option value={1}>原速记谱</option><option value={2}>半速记谱（适合快伴奏）</option></select></div><div><label htmlFor={`${prefix}-grid`}>最短时值</label><select id={`${prefix}-grid`} value={subdivisions} onChange={(e) => setSubdivisions(Number(e.target.value) as 2 | 4 | 8)}><option value={2}>八分音符</option><option value={4}>十六分音符</option><option value={8}>三十二分音符</option></select></div></>}
    </div>
    {notationEngine === "direct-jianpu" && <div className="field-stack"><label htmlFor={`${prefix}-keys`}>调号与转调 <small>可选；按小节填写，1 表示开头</small></label><input id={`${prefix}-keys`} value={keyChangesText} onChange={(e) => setKeyChangesText(e.target.value)} placeholder="例如 1:Eb, 56:E" /><small>人声按单旋律整理；伴奏保留叠音与独立时值。半速记谱保持原曲播放速度。</small></div>}
  </fieldset></details>;

  const loadJob = useCallback(async (jobId: string) => {
    const next = await readResponse(await fetch(`/api/v2/jobs/${jobId}`)) as Job;
    setJob(next);
    if (notationHydratedRef.current !== jobId) {
      const savedSelection = next.v2?.selection;
      setManualNotation(Boolean(savedSelection?.notation_advice?.version && (savedSelection.bpm_override_explicit || savedSelection.key_override_explicit || savedSelection.time_signature_override_explicit)));
      const settings = next.v2?.selection?.direct_options || next.v2?.direct_options || {};
      setNotationEngine(next.v2?.notation_engine === "direct-jianpu" ? "direct-jianpu" : "musescore-midi-import");
      setBeatDivisor(settings.beat_divisor || 1);
      setSubdivisions(settings.subdivisions || 4);
      setKeyChangesText((settings.key_changes || []).map((item) => `${item.bar}:${item.key}`).join(", "));
      notationHydratedRef.current = jobId;
    }
  ...
  useEffect(() => {
    ...
    const initialJob = resolveInitialJobId(window.location.search, window.localStorage.getItem("jianpu-v2-job-id"));
    if (initialJob.invalidQuery) {
      setError("链接中的任务 ID 无效，请使用 UUID 格式的 ?job=… 链接。");
      return;
    }
    if (initialJob.jobId) {
      setError("");
      void loadJob(initialJob.jobId).then(() => {
        window.localStorage.setItem("jianpu-v2-job-id", initialJob.jobId as string);
  ...
  const submit = async () => {
    ...
    try {
      const form = new FormData(); form.append("notation_engine", notationEngine); form.append("direct_options", JSON.stringify(directOptions())); form.append("file", file); form.append("source_kind", source); if (title.trim()) form.append("title", title.trim()); if (source === "vocal") form.append("separation_model", separationModel);
      ...
      window.history.replaceState(window.history.state, "", jobUrlForHistory(window.location.href, next.id));
  ...
  const generateVocal = async () => {
    ...
    try {
      const next = await readResponse(await fetch(`/api/v2/jobs/${job.id}/vocal/generate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ notation_engine: notationEngine, direct_options: directOptions() }) })) as Job;
  ...
  const submitSelection = async (kind: "midi" | "score") => {
    ...
    try {
      const payload = { notation_engine: notationEngine, direct_options: directOptions(), selected_track_ids: selectedTrackIds, merge_main_melody: mergeMainMelody, bpm_override: manualNotation ? Number(bpm) || 120 : undefined, key_override: manualNotation ? key : undefined, time_signature_override: manualNotation ? meter : undefined };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {job?.status === "failed" && primaryArtifactDownloads.length > 0 && <section className="failure-artifacts panel" aria-label="失败诊断产物"><div className="panel-heading"><div><p className="section-kicker">DIAGNOSTICS</p><h2>已生成的简谱文件</h2><p className="heading-note">本次已生成的 PDF、SVG 和 MIDI 可在这里下载。</p></div></div>{renderArtifactDownloads()}</section>}

      {isInstrumental && isReady && <section className="instrument-workspace panel" aria-labelledby="workspace-heading" data-selection-policy="single-checkbox-controls-roll-synth-midi-score"><div className="panel-heading workspace-title"><div><p className="section-kicker">04 · SELECTION READY</p><h2 id="workspace-heading">全量识别完成，选出要留下的轨道</h2><p className="heading-note">{trackCountLabel} · 勾选变化不会重新运行 MuScriptor。</p></div><span className="selection-revision">R{job?.v2?.selection_revision || 0}</span></div><div className="listen-strip"><div className="listen-card original-listen"><span className="listen-icon">◉</span><div><strong>原曲试听</strong><small>音频文件的真实播放</small></div>{originalArtifact?.url ? <audio ref={audioRef} controls src={originalArtifact.url} onTimeUpdate={(event) => setOriginalTime(event.currentTarget.currentTime)} onEnded={() => setOriginalTime(0)} /> : <span className="listen-pending">识别后可用</span>}</div><div className="listen-card synth-listen"><span className="listen-icon">⌁</span><div><strong>本机合成试听</strong><small title={soundfontStatus?.sha256 || "官方 SF3 音色库"} aria-live="polite">{synthResource}</small></div>{soundfontCacheNotice && <small className="synth-cache-note" aria-live="polite">{soundfontCacheNotice}</small>}<button type="button" className={`listen-button ${synthPlaying ? "playing" : ""}`} onClick={() => void playSynth()} disabled={synthLoading}>{synthLoading ? "准备试听…" : synthPlaying ? "暂停" : synthPaused ? "播放" : "播放选中轨"}</button>{synthLoading && soundfontDownloading && <button type="button" className="lightweight-action" onClick={useLightweightNow}>立即使用轻量试听</button>}<span className="synth-time">{formatDuration(synthTime)} / {formatDuration(duration)}</span>{synthError && <p className="synth-diagnostic" role="alert">{synthError}</p>}{lightweightActive && !synthLoading && <button type="button" className="lightweight-action" onClick={reloadHighQuality}>重新加载高质量音色</button>}</div></div><div className="workspace-grid"><div className="roll-column"><div className="subhead"><div><span className="section-kicker">PIANO ROLL</span><strong>时间同步试听轨</strong></div><div className="roll-tools"><label><input type="checkbox" checked={rollVisible} onChange={(event) => setRollVisible(event.target.checked)} />显示</label><label>缩放 <input aria-label="钢琴卷帘缩放" type="range" min="1" max="4" step="0.5" value={rollZoom} onChange={(event) => setRollZoom(Number(event.target.value))} /></label></div></div>{renderPianoRoll()}<p className="roll-caption">播放原曲或本机合成时，米白播放头沿原始秒数移动。颜色只表示轨道，音符时间不经过简谱量化。</p></div><aside className="track-roster"><div className="subhead"><div><span className="section-kicker">TRACK ROSTER</span><strong>乐器清单</strong></div><span className="roster-count">{selectedTrackIds.length}/{tracks.length}</span></div><div className="track-list">{tracks.map((track, index) => { const color = LANE_COLORS[index % LANE_COLORS.length]; const selected = selectedTrackIds.includes(track.track_id); return <div className={`track-row ${selected ? "selected" : ""}`} key={track.track_id}><span className="track-color" style={{ background: color }} /><label className="track-check"><input type="checkbox" checked={selected} onChange={() => toggleTrack(track.track_id)} /><span><strong>{track.label_zh}</strong><small>{track.instrument_group} · {track.note_count} notes · {track.is_drum ? "不生成简谱" : `program ${track.program}`}</small></span></label></div>; })}</div>{selectedTracks.some((track) => track.is_drum) && <p className="drum-note">鼓组保留在试听与选择 MIDI 中，但不会生成简谱。</p>}{!pitchedSelected && selectedTrackIds.length > 0 && <p className="drum-note">当前只选中鼓组：可以下载 MIDI，简谱按钮会保持禁用。</p>}</aside></div><div className="export-rail"><div className="analysis-advice" role="status"><div><strong>自动选择记谱方案</strong><span>{analysisSuggestion?.bpm || "—"} BPM · {analysisSuggestion?.key ? keyLabel(analysisSuggestion.key) : "—"} · {analysisSuggestion?.time_signature || "—"}</span></div>{analysisSuggestion?.notation_advice?.key_ambiguous && <small>调性有接近的候选，已采用当前得分最高的方案，可试听复核。</small>}<details><summary>选择依据与备选</summary><small>{analysisSuggestion?.notation_advice?.reason || "生成时会结合所选乐器重新比较拍速和调性。"}</small><small>拍速：{analysisSuggestion?.candidates?.bpm?.join(" / ")} · 调性：{analysisSuggestion?.candidates?.key?.map(keyLabel).join(" / ")}</small>{analysisSuggestion?.warnings?.map((warning, index) => <small key={index}>{warning}</small>)}</details></div><label className="merge-option"><input type="checkbox" checked={manualNotation} onChange={e => setManualNotation(e.target.checked)} />手动调整调性与速度（可选）</label><p>默认自动比较适合阅读的拍速和调性，按所选乐器生成。1= 表示数字 1 的音名，与 BPM 无关。</p>{manualNotation && <div className="override-fields"><div><label htmlFor="export-bpm">BPM</label><input id="export-bpm" inputMode="decimal" value={bpm} onChange={(event) => updateSelectionField("bpm", event.target.value)} /></div><div><label htmlFor="export-key">调性</label><select id="export-key" value={key} onChange={(event) => updateSelectionField("key", event.target.value)}>{KEYS.map((item) => <option key={item} value={item}>{keyLabel(item)}</option>)}</select></div><div><label htmlFor="export-meter">拍号</label><select id="export-meter" value={meter} onChange={(event) => updateSelectionField("meter", event.target.value)}>{METERS.map((item) => <option key={item}>{item}</option>)}</select></div></div>}<label className="merge-option"><input type="checkbox" checked={mergeMainMelody} onChange={(event) => setMergeMainMelody(event.target.checked)} />另外生成单声部主旋律 <small>会丢失和声</small></label><div className="export-actions"><button type="button" className="primary-action compact" onClick={() => void submitSelection("score")} disabled={busy || !pitchedSelected}><span>生成分谱</span><strong>↗</strong></button></div></div>{renderHighAccuracyMeta()}{(melodyHarmonyLongScoreArtifacts.length > 0 || melodyHarmonyPagedScoreArtifacts.length > 0 || melodyHarmonyMidi) && <div className="score-deck melody-harmony-deck" data-score-family="melody-harmony"><div className="subhead"><div><span className="section-kicker">MELODY + HARMONY</span><strong>{job?.v2?.notation_engine === "direct-jianpu" ? "组合谱：高音、和声与低音独立分层" : "组合谱：主旋律上方，伴奏和弦下方"}</strong></div><span className="score-warning">当前 R{job?.v2?.selection_revision || 0}</span></div>{renderScoreCards(melodyHarmonyLongScoreArtifacts, true, "组合长图 SVG · 默认预览")}{renderPagedScoreDetails(melodyHarmonyPagedScoreArtifacts, "分页 SVG · 主旋律在上，伴奏和弦在下")}{melodyHarmonyLongScoreArtifacts.map((artifact) => <a className="download-line" key={`${artifact.artifact_id}-download`} href={artifact.url || "#"}>↓ 下载组合长图 SVG · {artifact.label}</a>)}{melodyHarmonyMidi && <a className="download-line combined-midi-download" href={melodyHarmonyMidi.url || "#"} download={melodyHarmonyMidi.filename}>↓ 下载组合谱 MIDI · 主旋律 + 伴奏和弦 · R{job?.v2?.selection_revision || 0}</a>}</div>}{(longScoreArtifacts.length > 0 || pagedScoreArtifacts.length > 0) && <div className="score-deck" data-score-family="instrument"><div className="subhead"><div><span className="section-kicker">INDEPENDENT PARTS</span><strong>已选轨道独立分谱（原始完整分谱）</strong></div><span className="score-warning">组合谱另列；鼓组不在分谱中</span></div>{renderScoreCards(longScoreArtifacts, true, "独立轨道纵向长图 SVG · 默认预览")}{renderPagedScoreDetails(pagedScoreArtifacts, "独立轨道分页 SVG · 下载或新窗口查看")}{longScoreArtifacts.map((artifact) => <a className="download-line" key={`${artifact.artifact_id}-download`} href={artifact.url || "#"}>↓ 下载独立分谱长图 SVG · {artifact.label}</a>)}</div>}{(mainMelodyLongScoreArtifacts.length > 0 || mainMelodyPagedScoreArtifacts.length > 0 || mainMelodyMidi) && <div className="score-deck main-melody-deck" data-score-family="main-melody"><div className="subhead"><div><span className="section-kicker">OPTIONAL MELODY</span><strong>可选单音主旋律</strong></div><span className="score-warning">单声部；会丢失和声</span></div>{renderScoreCards(mainMelodyLongScoreArtifacts, true, "单音主旋律纵向长图 SVG · 仅旋律")}{renderPagedScoreDetails(mainMelodyPagedScoreArtifacts, "单音主旋律分页 SVG · 仅旋律")}{mainMelodyLongScoreArtifacts.map((artifact) => <a className="download-line" key={`${artifact.artifact_id}-download`} href={artifact.url || "#"}>↓ 下载单音主旋律长图 SVG · {artifact.label}</a>)}{mainMelodyMidi && <a className="download-line" href={mainMelodyMidi.url || "#"} download={mainMelodyMidi.filename}>↓ 下载单音主旋律 MIDI · R{job?.v2?.selection_revision || 0}</a>}</div>}{renderArtifactDownloads()}</section>}
